...
//数据库配置
    ...
    while ($row = mysql_fetch_assoc($r_province)) {
        ...
        echo '<h3>省份: ' . $re_pro['name'] . '(总数' . $content_num_province['num'] . ')</h3>';
		$j = $j+$content_num_province['num'];
        #查询市
        ...
        while ($row = mysql_fetch_assoc($r_city)) {
            ...
            echo "<li style='float:left;margin-left:15px;'>" . $re_city['name'] . "(" . $content_num_city['num'] . ")</li>";
        }
        echo "</ul>";
