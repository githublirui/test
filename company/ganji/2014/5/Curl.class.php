...
class Curl {
    ...
    public $mTimeout = 3;    //设置curl允许执行的最长秒数
    public $cTimeout = 2; //设置curl等待连接的超时,php5.2.3可用
    ...
    function post($url, $post = null, $updatefile = false) {
        ...
        if (is_array($this->mHttpHeader))
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->mHttpHeader);
        //{{{ ssl
    ...
    public function get($url) {
        ...
        // @codeCoverageIgnoreEnd
        if (null != $this->mReferer)
            curl_setopt($ch, CURLOPT_REFERER, $this->mReferer);
        if (is_array($this->mHttpHeader))
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->mHttpHeader);
        $msg = "请求url:" . $url;
        $html = curl_exec($ch);
    ...
    function log($msg, $logfile = null) {
        return;
        // 如果目录不存在就不记录log了
        if (false == file_exists($this->logPath)) {
            return;
        }
        ...
        //       echo $msg;
        if (empty($logfile))
            $logfile = $this->logPath . "/curl_" . $filedate . ".log";
