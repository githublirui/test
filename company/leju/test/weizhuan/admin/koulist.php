...
$max=$page*35;
$row=$mysql->query("select * from `koudata` order by `id` desc limit {$min},{$max}");
//查看分享记录
$uid=guolv($_GET['uid']);
if(is_numeric($uid)){
	$row=$mysql->query("select * from `koudata` where `uid`='{$uid}' order by `id` desc limit {$min},{$max}");
}

//查看日期
$day=guolv($_GET['day']);
if(is_date($day)){
	$row=$mysql->query("select * from `koudata` where `day`='{$day}' order by `id` desc limit {$min},{$max}");
}

//查看ip
$kip=guolv($_GET['ip']);
if(is_ip($kip)){
	$row=$mysql->query("select * from `koudata` where `ip`='{$kip}' order by `id` desc limit {$min},{$max}");
}
...
<div class="well">
	...
	<div class="btn-group pull-right">
		<a href="koulist.php" class="btn">首页</a>
		<a href="?page=<?php echo $page_1?>" class="btn" style="margin-left:5px;">上一页</a>
		<a href="?page=<?php echo $page_2?>" class="btn" style="margin-left:5px;">下一页</a>
	</div>			
    <table class="table">
      <thead>
        <tr>
...
print <<<table
	  ...
	  <td><a href="?ip={$v_referer['ip']}">{$v_referer['ip']}</a></td>
		  ...
		  <td>
		  <span ip="{$v_referer['ip']}"><a href="#">归属地</a></span>
		  </td>
		  <td>{$time}</td>
        </tr>
