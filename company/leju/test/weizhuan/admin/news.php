...
if($_POST){
	//新闻发布
		...
		if($news_title!=='' && $news_content!==''){
			$mysql->query("insert into `newsdata` values(null,'{$news_title}','{$news_content}')");
			$id=mysql_insert_id();
			if($id!==0){
...
<div class="well">
    ...
    <form id="tab" action="news.php" method="post">
        <label>新闻标题</label>
        <input type="text" value="" style="width:500px" name="news_title">
        <textarea value="Smith" rows="10" name="news_content" style="width:800px;height:400px;visibility:hidden;"></textarea>
		 <label></label>
