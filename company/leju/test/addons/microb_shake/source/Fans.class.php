...
class Fans {
    ...
    public function getOne($uid, $isOpenid = false) {
        ...
        if($isOpenid){
            ...
            $pars[':uid'] = intval($uid);
            $sql = 'SELECT * FROM ' . tablename('mbsk_fans') . ' WHERE `uniacid`=:uniacid AND `uid` =:uid';
        }
    ...
    public function getAll($filters = array(), $pindex = 0, $psize = 15, &$total = 0) {
        ...
        $sql = 'FROM ' . tablename('mbsk_fans') . ' AS `f` LEFT JOIN ' . tablename('mbsk_records') . ' AS `r` ON (`f`.`uid`=`r`.`uid`) WHERE ';
        $sql .= $condition;
        if($pindex > 0){
        ...
        if(!empty($ds)) {
            foreach($ds as &$row) {
                $row['helps'] = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('mbsk_helps') . ' WHERE `uniacid`=:uniacid AND `from`=:uid', array(':uniacid' => $_W['uniacid'], ':uid' => $row['uid']));
            }
            unset($row);
        }
        return $ds;
    ...
    public function getProfile($uid) {
        ...
        if(!empty($cache[$uid])) {
            ...
            $sql = "SELECT * FROM " . tablename('mbsk_profiles') . " WHERE `uid`=:uid AND `uniacid`=:uniacid";
            $pars = array();
            $pars[':uid'] = $uid;
            $pars[':uniacid'] = $_W['uniacid'];
