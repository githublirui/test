...
class Teabox_pennyModuleSite extends WeModuleSite {	
    ...
    public function doMobilePay() {
		...
		$setting = pdo_fetch('SELECT * FROM ' . tablename('teabox_penny_setting') . ' WHERE `uniacid` = :uniacid', array(':uniacid' => 9));
        ...
        $url ="http://www.hfwxz.com/app/index.php?i=9&c=entry&do=auth&dopenid=$dopenid&m=teabox_penny";
		...
		if (!$user['dopenid']){
			if($dopenid){
				...
				pdo_update('teabox_penny_user', $d, array('id' => $user['id']));
			}else{
				...
				if($payd){
					include $this->template('nofollow');
				}else{
					include $this->template('nofollow');
				}
				exit;
...
//		//设置每日迎娶小薇的人数
//		if($total>1000){
//			echo '今天排队迎娶小微的人太多了，请明天再来吧，少年！';exit;
//		}
		$user = pdo_fetch('SELECT * FROM ' . tablename('teabox_penny_user') . ' WHERE `uniacid` = :uniacid AND `id` = :id', array(':uniacid' => $_W['uniacid'],':id' => $uid ) );

		if($user['province'] !='浙江' && $uid!='409'){
			include $this->template('tofar');exit;
			echo '亲您的朋友圈太远了，（提示：活动仅限浙江区域小伙伴参加哦！）';exit;
		}
		
		
		//if(!$pay){
			if($_GPC['money']=='a'){
				$total_fee = rand(100,150)/100;
			}elseif($_GPC['money']=='b'){
				$total_fee = rand(150,200)/100;
			}elseif($_GPC['money']=='c'){
				$total_fee = rand(200,250)/100;
			}elseif($_GPC['money']=='d'){
				$total_fee = rand(250,300)/100;
			}
...
//        );
    ...
    public function doMobileAuth() {
		...
		$openid = $_GPC['dopenid'];
        if(empty($_GPC['code'])){
            $back_url ="http://www.hfwxz.com/app/index.php?i=9&c=entry&do=auth&dopenid=$openid&m=teabox_penny";
