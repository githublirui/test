...
class PingtuModuleSite extends WeModuleSite {
    ...
    function send($open_id, $fee, $record_id) {
        ...
        $hongbao_mchid = '1271329001';
        $hongbao_appid = 'wxc4503dec164e0e32';
        $hongbao_provider = '宁波万科';
        $hongbao_wish = '宁波万科';
        $hongbao_ip = '121.40.83.102';
        $hongbao_title = '宁波万科';
        $hongbao_remark = '宁波万科';
        $hongbao_password = 'z0f1rrn5usjadtwfbthrxl2ozpl4bbfu';
        ...
        $extras = array();
        $extras['CURLOPT_CAINFO'] = '/var/www/html/addons/pingtu/cert/rootca.pem';
        $extras['CURLOPT_SSLCERT'] = '/var/www/html/addons/pingtu/cert/apiclient_cert.pem';
        $extras['CURLOPT_SSLKEY'] = '/var/www/html/addons/pingtu/cert/apiclient_key.pem';
    ...
    public function doMobileAuth() {
        ...
        $code = $_GPC['code'];
        load()->func('communication');
        $url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=wxc4503dec164e0e32&secret=525df51f5f8834901d7ef76540745a09&code={$code}&grant_type=authorization_code";
        $resp = ihttp_get($url);
        if (is_error($resp)) {
        ...
        if (is_array($auth) && !empty($auth['openid'])) {
            ...
            if (is_array($info) && !empty($info['openid'])) {
                ...
                $sql = 'SELECT * FROM ' . tablename('xhw_pingtu_info') . ' WHERE `uniacid`=' . $_W['uniacid'] . ' AND `openid` ="' . $info['openid'] . '"';
                $ret = pdo_fetch($sql);
                if (!$ret) {
                ...
                $_SESSION['__:proxy:openid'] = $info['openid'];
    ...
    protected function auth() {
        ...
        $forward = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc4503dec164e0e32&redirect_uri={$callback}&response_type=code&scope=snsapi_userinfo&state={$stateKey}#wechat_redirect";
        header('Location: ' . $forward);
        exit;
    }
...
//判断结束
        ...
        $slides['0']['desc'] = '拼图赢红包 万元大派送';
        $slides['0']['url'] = $_W['siteroot'] . 'app' . substr($this->createMobileUrl('index'), 1);
        $slides[0][icon] = $_W['siteroot'] . 'attachment/' . $slides[0][icon];
...
//        unset($cwx);
        ...
        //更新分享次数
        #1. 查询用户是否存在，不存在插入用户,保存已分享
        $registUser = pdo_fetch("select * from " . tablename("xhw_pingtu_user") . " where info_id ={$user['id']}");
        if (!$registUser) {
            ...
            pdo_insert('xhw_pingtu_user', $insert_user_data);
        } else {
            #2. 更新分享次数
            $sql = "update " . tablename('xhw_pingtu_user') . " set share_num=share_num+1 where info_id=" . $user['id'];
            pdo_query($sql);
        }

        if (!$registUser['tel'] || $registUser['score'] <= 0) {
            echo '亲，玩完游戏，注册手机号可以领取红包';
            exit;
        }
        ...
        $sql = 'SELECT * FROM ' . tablename('xhw_pingtu_info') . ' WHERE `uniacid`=' . $_W['uniacid'] . ' AND `openid` ="' . $user['openid'] . '" and fee !="" and status = "complete"';
        $ret = pdo_fetch($sql);
        if ($ret) {
            $res['msg'] = "您已经领过红包了";
        } else {
...
//            $rec['status'] = '';
            ...
            $res['msg'] = "恭喜你获得{$rec['fee']}元红包。红包将在几分钟内收到，请耐心等待";
...
//        include $this->template('gethb');
    ...
    public function doMobileGetIndex() {
        ...
        if ($registUser) {
            //已经存在更新用户信息
            pdo_update('xhw_pingtu_user', $data, array("id" => $registUser["id"]));
            $id = $registUser['id'];
        } else {
