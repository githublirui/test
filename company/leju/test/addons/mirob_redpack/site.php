...
class Microb_RedpackModuleSite extends WeModuleSite {
    ...
    protected function prepareActivity($activity, $verifyParams = array()) {
        ...
        if($activity['end'] < TIMESTAMP) {
            return error(-2, '<h4>手慢了, 本次活动已经结束, 请期待下次活动</h4> <br><h5>本次活动时间为: <br>' . date('Y-m-d H:i', $activity['start']) . ' <br> ' . date('Y-m-d H:i', $activity['end']) . '</h5>');
        }
        if(!empty($activity['limit'])) {
            ...
            if(!empty($activity['limit']['gender']) && !empty($verifyParams['user'])) {
                $gender = '未知';
                if($verifyParams['user']['gender'] == '1') {
                    $gender = '男';
                }
                if($verifyParams['user']['gender'] == '2') {
                    $gender = '女';
                }
                if($activity['limit']['gender'] != $gender) {
            ...
            if(!empty($activity['limit']['amount'])) {
                if($activity['amount'] >= $activity['limit']['amount']) {
                    return error(-4, "<h4>哎呀, 手慢了. 本次活动所有红包已发完</h4> <br><h5>本次活动已经结束, 请期待我们下一次活动</h5>");
                }
            }
        }
