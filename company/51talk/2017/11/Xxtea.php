...
//static
    ...
    var redir = {key: "", url: "http://web.oa.51talk.com/", withtoken: false, redirect: true, token: "", loginerror: ""};

    pwd = (new Xxtea('b398cb18c9e9419498f420b223001633')).xxtea_encrypt('649037629@qq.com');
    
    (function (config) {
        if (config.redirect && config.url != "" && config.url != null && config.url != "Null") {
            if (config.withtoken && config.token != null && config.token != "") {
                if (config.url.indexOf("?") > 0) {
                    document.location.href = config.url + "&token=" + config.token;
                } else {
                    document.location.href = config.url + "?token=" + config.token;
                }
            } else {
