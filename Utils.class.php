...
//        $text = preg_replace('/\W/', ' ', $text);
     ...
     * 
     */
    public static function msubstr($string, $start, $length, $charset = 'utf-8') {
        $chars = $string;
        $m = 0;
        $n = 0;
        ...
        $str1 = mb_substr($string, $start, $k, $charset);
        return $str1;
    }
    ...
    public static function distance_of_time_in_words($from_time, $to_time = null, $include_seconds = false) {
        ...
        if ($distance_in_minutes <= 1) {
            if (!$include_seconds) {
                $string = $distance_in_minutes == 0 ? '小于1分钟' : '1 分钟';
            } else {
            ...
            $string = '%分钟% 分钟';
            $parameters['%分钟%'] = $distance_in_minutes;
        } else if ($distance_in_minutes >= 45 && $distance_in_minutes <= 89) {
            $string = '大概一小时';
        } else if ($distance_in_minutes >= 90 && $distance_in_minutes <= 1439) {
            $string = '大概 %小时% 小时';
            $parameters['%小时%'] = round($distance_in_minutes / 60);
        } else if ($distance_in_minutes >= 1440 && $distance_in_minutes <= 2879) {
            $string = '1 天';
        } else if ($distance_in_minutes >= 2880 && $distance_in_minutes <= 43199) {
            $string = '%天% 天';
            $parameters['%天%'] = round($distance_in_minutes / 1440);
        } else if ($distance_in_minutes >= 43200 && $distance_in_minutes <= 86399) {
            $string = '大概 1 个月';
        } else if ($distance_in_minutes >= 86400 && $distance_in_minutes <= 525959) {
            $string = '%月% 月';
            $parameters['%月%'] = round($distance_in_minutes / 43200);
        } else if ($distance_in_minutes >= 525960 && $distance_in_minutes <= 1051919) {
            $string = '大概一年';
        } else {
            $string = '超过 %年% 年';
            $parameters['%年%'] = floor($distance_in_minutes / 525960);
        }
     ...
     * @return boolean
     */
    public static function getTheUrlIsActive($urls) {
        $result = '';
        if (is_array($urls)) {
            $result = in_array($_SERVER['REQUEST_URI'], $urls);
        } else {
            $result = $_SERVER['REQUEST_URI'] == $urls;
        }
        return $result;
    }
